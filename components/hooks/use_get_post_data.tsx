  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
         ...
         * Uses mainnet if NEXT_PUBLIC_NETWORK is "mainnet", otherwise defaults to testnet
         */
        const mirrorNodeUrl =
        ...
        const response = await fetch(
          `${mirrorNodeUrl}/api/v1/topics/${process.env.NEXT_PUBLIC_EXPLORER_ID}/messages/${sequenceNumber}`
        );
        ...
        if (!response.ok) {
          throw new Error(`HTTP error on ${networkType}! Status: ${response.status}`);
        }
        ...
        setError(`Failed to fetch post data from ${networkType}`);
        toast.error(`Failed to fetch post data from ${networkType}: ${errorMessage}`);
      } finally {
        setLoading(false);
      }
    };
