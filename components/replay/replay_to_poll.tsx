...
interface ReplayProps {
  ...
  votedChoice?: string | null; // The choice the user voted for (if any)
...
const ReplayPoll: React.FC<ReplayProps> = ({
  ...
  const reactionTarget = isComment ? "this comment" : "this poll";
  ...
  const generateShareLink = () => {
    ...
    if (isComment) {
      return `${baseUrl}/Polls/${topicId}?comment=${sequenceNumber}`;
    }
    // Otherwise, generate poll link
    return `${baseUrl}/Polls/${topicId}`;
  };
  ...
  const handlePoll = async () => {
    ...
    if (hasVoted) {
      const optionName = votedChoice?.replace("Choice", "Option ") || "an option";
      toast.warning(`You have already voted for ${optionName}. Only one vote per user is counted.`);
      return;
    }
    if (Choice === "") {
    ...
    <>
      ...
      <div className="flex flex-wrap gap-1.5 sm:gap-2 sm:flex-nowrap">
        ...
        <div className="flex gap-1.5 sm:gap-2 flex-wrap sm:flex-nowrap">
          ...
          {showVoteButton && (
            <button
              className={`group flex items-center justify-center gap-1 px-2 py-1.5 sm:px-3 sm:py-2 sm:gap-2 rounded-full transition-all duration-300 border min-w-[44px] sm:min-w-[60px] ${
                hasVoted
                  ? "bg-green-500 text-white border-green-400/50 cursor-default shadow-lg shadow-green-500/25"
                  : Choice === ""
                  ? "bg-slate-700/60 text-slate-400 border-slate-600/50 cursor-not-allowed"
                  : "bg-gradient-to-r from-cyan-400 to-blue-500 hover:scale-105 text-white shadow-lg shadow-cyan-400/25 border-cyan-400/30 hover:border-cyan-400/50"
              }`}
              onClick={handlePoll}
              disabled={hasVoted}
              title={
                hasVoted
                  ? `You voted for ${votedChoice?.replace("Choice", "Option ") || "this poll"}`
                  : Choice === ""
            ...
            >
              {hasVoted ? (
                <>
                  ...
                  <span className="text-xs font-mono font-medium hidden sm:inline">Voted</span>
                </>
              ) : (
                <span className="text-xs font-mono font-medium">Vote</span>
              )}
            </button>
          )}
        </div>
