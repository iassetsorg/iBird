/**
 * Public Group View Page
 * Allows any user to view a group via shareable URL: /group/[id]
 * Logged-in users can follow/unfollow the group (if not owner)
 */
...
import useFollow, { StepStatus, AddItemStepCallbacks, FollowedGroup } from "../../components/hooks/use_follow";
...
import { NewMessage } from "../../components/send message/new_message";
...
export default function PublicGroupPage() {
    ...
    const [pendingFollowData, setPendingFollowData] = useState<FollowedGroup | null>(null);
    ...
    const getFirstFollowStep = (): keyof FollowSteps => {
        return isFirstGroupFollow() ? "createListTopic" : "sendToList";
    };
    ...
    const startFollowProcess = async () => {
        ...
        const result = await followGroupWithSteps(pendingFollowData, stepCallbacks);
    ...
    // V1 profiles have Groups as an array, V2 profiles have Groups as a topic ID string
    // For V2 profiles, we need to check differently (ownership determined by comparing creator account)
    const isOwner = Array.isArray(profileData?.Groups)
        ? (profileData.Groups as Array<{ Group: string }>).some(
            (group) => group.Group === groupId
          )
        : false; // For V2 profiles, ownership check needs group data from topic (handled separately)

    useEffect(() => {
        if (!groupId) return;
        setIsFollowingLocal(isFollowingGroup(groupId));
    }, [groupId, profileData, isFollowingGroup]);

    // Fetch group metadata - get topic info from mirror node + creator's profile for Media
    useEffect(() => {
        const fetchMetadata = async () => {
            ...
            try {
                ...
                // First, try to get topic info (contains memo with group name)
                const topicResponse = await fetch(
                    `${mirrorNodeUrl}/api/v1/topics/${groupId}`
                );

                if (topicResponse.ok) {
                    const topicData = await topicResponse.json();
                    // Memo format is "Group: GroupName"
                    const memo = topicData.memo || "";
                    groupName = memo.startsWith("Group: ")
                        ? memo.replace("Group: ", "")
                        : memo || "Group";
                }
                ...
                const msgResponse = await fetch(
                    `${mirrorNodeUrl}/api/v1/topics/${groupId}/messages?order=asc&limit=1`
                );
                ...
                if (msgResponse.ok) {
                    ...
                    if (msgs.length > 0) {
                        ...
                        try {
                            ...
                            if (metadata.Name) groupName = metadata.Name;
                            if (metadata.Description) groupDescription = metadata.Description;
                            if (metadata.Media) groupMedia = metadata.Media;
                        } catch {
                            // First message isn't JSON metadata, that's fine
                        }

                        // If we still don't have Media, try to fetch creator's profile
                        if (!groupMedia && creatorAccountId) {
                            try {
                                        ...
                                        try {
                                            ...
                                            if (profileResponse.ok) {
                                                ...
                                                if (profileMessages.length > 0) {
                                                    ...
                                                    // Find this group in creator's profile
                                                    const groupsArray = profileData.Groups || [];
                                                    const foundGroup = groupsArray.find(
                                                        (g: { Group: string; Media?: string; Description?: string }) =>
                                                            g.Group === groupId
                                                    );

                                                    if (foundGroup) {
                                                        if (foundGroup.Media) groupMedia = foundGroup.Media;
                                                        if (foundGroup.Description && !groupDescription) {
    ...
    const handleScroll = useCallback(
        ...
        [groupId, scrollTop, isRefreshing, loading]
    );
    ...
    const handleFollow = async () => {
        ...
        const groupData = {
            Name: groupMetadata?.Name || "Group",
            Group: groupId,
            Description: groupMetadata?.Description || "",
            Media: groupMetadata?.Media || "",
        };
        ...
        if (isV2Profile) {
            ...
            setFollowStarted(false);
            setAutoProgressFollowDisabledByError(false);
            const isFirstFollow = isFirstGroupFollow();
            setPendingFollowData(groupData);
            ...
            const success = await followGroup(groupData);
            if (success) {
                setIsFollowingLocal(true);
            }
        }
    ...
    const handleUnfollow = async () => {
        const success = await unfollowGroup(groupId);
        if (success) {
    ...
    // Check if following
    const isFollowing = isFollowingLocal || isFollowingGroup(groupId);
    ...
    return (
        <>
            <Head>
                <title>
                    {groupMetadata?.Name || "Group"} - iBird
                </title>
                <meta
                    name="description"
                    content={groupMetadata?.Description || "View group on iBird"}
                />
            </Head>
            ...
            <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900">
                ...
                <div className="min-h-screen flex flex-col max-w-4xl mx-auto px-4 pt-20 pb-8">
                    {/* Group Header Card */}
                    <div className="bg-gradient-to-br from-slate-800/80 to-slate-900/80 backdrop-blur-md rounded-2xl border border-purple-400/20 shadow-xl shadow-purple-400/5 mb-6 overflow-hidden">
                        {/* Header Top */}
                        <div className="bg-gradient-to-r from-purple-400/10 via-pink-500/10 to-purple-400/10 px-6 py-5">
                            <div className="flex items-center justify-between flex-wrap gap-4">
                                <div className="flex items-center gap-4">
                                    <button
                                        onClick={() => router.back()}
                                        className="p-2.5 rounded-xl bg-purple-400/10 hover:bg-purple-400/20 text-purple-400 transition-all duration-200 hover:scale-105"
                                        aria-label="Go back"
                                    >
                                    ...
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div className="relative flex-shrink-0">
                                            {isMetadataLoading ? (
                                                ...
                                                <div className="w-14 h-14 rounded-xl overflow-hidden ring-2 ring-purple-400/40">
                                                    <ReadMediaFile cid={groupMetadata.Media} />
                                                </div>
                                            ) : (
                                                <div className="w-14 h-14 rounded-xl bg-gradient-to-br from-purple-400 to-pink-500 flex items-center justify-center shadow-lg">
                                                    <RiGroupLine className="w-7 h-7 text-white" />
                                                </div>
                                            )}
                                        ...
                                        <div>
                                            <h1 className="text-xl sm:text-2xl font-bold text-white">
                                                {isMetadataLoading ? (
                                                    ...
                                                    groupMetadata?.Name || "Unnamed Group"
                                                )}
                                            </h1>
                                            <div className="flex items-center gap-2 mt-1 flex-wrap">
                                                <span className="inline-flex items-center px-2.5 py-1 rounded-lg text-xs font-semibold bg-purple-400/20 text-purple-300 border border-purple-400/30">
                                                    <RiGroupLine className="w-3 h-3 mr-1" />
                                                    Group
                                                </span>
                                                {isOwner && (
                                ...
                                <div className="flex items-center gap-3">
                                    ...
                                    <button
                                        onClick={handleShare}
                                        className="px-4 py-2.5 rounded-xl bg-purple-400/10 hover:bg-purple-400/20 text-purple-400 border border-purple-400/30 transition-all duration-200 flex items-center gap-2 font-medium"
                                        title="Share group"
                                    >
                                        ...
                                        <>
                                            {isFollowing ? (
                                                ...
                                                <button
                                                    ...
                                                    disabled={isFollowLoading}
                                                    className="px-4 py-2.5 rounded-xl bg-gradient-to-r from-purple-400 to-pink-500 hover:from-purple-300 hover:to-pink-400 text-white transition-all duration-200 flex items-center gap-2 font-medium text-sm shadow-lg hover:shadow-purple-400/30 disabled:opacity-50"
                                                >
                                                    {isFollowLoading ? (
                                                        <Spinner />
                                                    ) : (
                    ...
                    >
                        ...
                        {isRefreshing && (
                            <div className="flex items-center justify-center py-4 bg-purple-900/20 border-b border-purple-400/20">
                                <Spinner />
                                <span className="ml-2 text-sm text-purple-400 font-mono">Refreshing...</span>
                            </div>
                        )}
                        ...
                        {loading && allMessages.length === 0 && (
                            <div className="flex h-64 items-center justify-center">
                                <div className="text-center space-y-4">
                                    <div className="w-16 h-16 mx-auto rounded-2xl bg-purple-400/10 flex items-center justify-center border border-purple-400/20">
                                        <Spinner />
                                    </div>
                        ...
                        {!loading && allMessages.length === 0 && (
                            <div className="flex flex-col items-center justify-center py-20">
                                <div className="w-20 h-20 rounded-2xl bg-purple-400/10 flex items-center justify-center mb-6 border border-purple-400/20">
                                    <RiGroupLine className="w-10 h-10 text-purple-400/50" />
                                </div>
                                <h3 className="text-xl font-bold text-white/90 mb-3">
                                    No Messages Yet
                                </h3>
                                <p className="text-white/60 text-sm text-center max-w-md px-4">
                                    This group is empty. Messages will appear here once members start posting.
                                </p>
                            </div>
                        )}
                                ...
                                return (
                                    <div key={message.message_id} className="p-5 hover:bg-slate-700/20 transition-colors">
                                        <div className="flex items-start justify-between gap-4 mb-3">
                                            <span className="px-2 py-1 bg-purple-400/15 text-purple-300 text-xs font-mono rounded-md border border-purple-400/20">
                                                {message.Type || "Post"}
                                            </span>
                        ...
                        {isLoadingMore && nextLink && (
                            <div className="flex items-center justify-center py-6 border-t border-slate-700/50">
                                <Spinner />
                                <span className="ml-2 text-sm text-purple-400/70 font-mono">Loading more...</span>
                            </div>
                        )}
                    </div>
                </div>
                ...
                {(isOwner || isFollowing) && (
                    <NewMessage targetTopicId={groupId} />
                )}

                {/* Follow Progress Modal */}
                {showFollowProgressModal && (
                    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                        <div className="bg-gradient-to-br from-slate-800 to-slate-900 rounded-2xl border border-purple-400/30 shadow-2xl shadow-purple-500/20 max-w-md w-full">
                            {/* Modal Header */}
                                ...
                                <div className="flex items-center justify-between">
                                    <div className="flex items-center gap-3">
                                        <div className="w-10 h-10 rounded-xl bg-purple-400/20 flex items-center justify-center">
                                            <RiUserFollowLine className="w-5 h-5 text-purple-400" />
                                        </div>
                                        <div>
                                            <h3 className="text-lg font-bold text-white">
                                                {followComplete ? "Following Group" : "Follow Group"}
                                            </h3>
                                ...
                                <div className="flex items-center gap-2 mb-4">
                                    ...
                                    <div className="flex items-center gap-2">
                                        <button
                                            ...
                                            className={`relative w-11 h-6 rounded-full transition-colors duration-200 ${autoProgressFollow ? "bg-purple-500" : "bg-slate-600"}
                            ...
                            <div className="p-6 space-y-4">
                                {isFirstGroupFollow() && renderFollowStepButton(
                                    "createListTopic",
                                    "Create Following List",
                                    "Create topic to store followed groups",
                                    "1"
                                )}

                                {renderFollowStepButton(
                                    "sendToList",
                                    isFirstGroupFollow() ? "Add Group to List" : "Update Following Groups",
                                    "Save group to your following list",
                                    isFirstGroupFollow() ? "2" : "1"
                                )}
                                    ...
                                    <div className="mt-2 p-4 rounded-xl bg-green-500/10 border border-green-500/30">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div>
                                                ...
                                                <p className="text-xs text-green-400/70 mt-1">You are now following this group</p>
                                            </div>
                                        </div>
                                    </div>
                                )}
                            ...
                            <div className="p-6 border-t border-purple-400/20">
                                {followComplete ? (
                                    <button
                                        onClick={() => setShowFollowProgressModal(false)}
                                        className="w-full py-3 rounded-xl bg-gradient-to-r from-purple-400 to-pink-500 text-white font-semibold hover:from-purple-300 hover:to-pink-400 transition-all"
                                    >
                                        Done
                                    </button>
                                ) : followError ? (
                                    <div className="flex gap-3">
                                        ...
                                        <button
                                            ...
                                            className="flex-1 py-3 rounded-xl bg-purple-500 text-white font-semibold hover:bg-purple-400 transition-colors"
                                        >
                                            Retry
                                        </button>
                                    </div>
